# Day 3 Phase 026 - Raw Path And Secret Leak Audit
...
## Objective
- Remove owner-facing raw local path exposure from Gateway provider APIs and verify no secret-shaped values appear.
## Actions Completed
- Found authenticated provider API payloads were exposing raw local endpoint paths.
- Added a recursive sanitizer for Bridge provider payloads.
- Applied sanitizer to provider list and provider detail routes.
- Added focused tests for raw local path and secret-shaped value redaction.
- Restarted Mission Control and reran route smoke.
...
## Proof
- Before fix: provider payloads exposed raw local endpoint paths in authenticated JSON.
- After fix: provider list and provider detail routes returned 200 with raw_path_visible=false and secret_shape_visible=false.
- Authenticated /api/bridge/providers: 200, raw_path_visible=false, secret_shape_visible=false.
- Authenticated /api/bridge/providers/hermes: 200, raw_path_visible=false, secret_shape_visible=false.
- Authenticated /api/gateway/status: 200, raw_path_visible=false, secret_shape_visible=false.
- Authenticated /api/gateway/agent-hub/status: 200, raw_path_visible=false, secret_shape_visible=false.
...
## Exact Next Step
- Apply the same sanitizer pattern to any future provider-like payload before exposing it through Gateway APIs.
