# Developer Continuation List

## Owner-Only Blockers
- Owner-authenticated browser visual proof.
- Paperclip owner login/session bridge.
- Telegram live owner prompt proof and possible BotFather rename.
- Bridge Session approval for any writes/uploads/Run Now.

## Credential Or Connector Blockers
- Firecrawl live credential/backend.
- YouTube transcript connector.
- Google Drive upload connector.
- OneDrive upload connector.
- Telegram PDF attachment route.
- AgentMail allowed-domain send.

## Provider Or Runtime Blockers
- Hermes safe live adapter must be configured until hermes_called:true.
- Pi standalone runtime/session not found.
- OpenClaw+ doctor unhealthy issues remain.
- n8n not installed.

## Future Expansion
- Broaden owner-auth browser proof into a screenshot regression pack.
- Convert Paperclip dry-run task tracking into live task flow after owner login.
- Add provider-specific reporting after credentials are approved.
...
## Next Step
Start a focused blocker-clearing workstream with owner sessions and credentials available.
