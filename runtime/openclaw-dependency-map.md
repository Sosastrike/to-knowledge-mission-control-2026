...
| Mission Control task dispatch transport | `src/lib/task-dispatch.ts`; `src/app/api/tasks/[id]/broadcast/route.ts` | Legacy task dispatch, quality review, targeted-session send, and task broadcast transport | Could keep OpenClaw as hidden task/message dispatcher if not migrated to direct agent lines | migrate_to_nuclear_gateway | Direct Agent Line task dispatch and broadcast adapters with route trace, audit, and rollback/no-state proof | No runtime behavior changed in inventory hop; code rollback via `git revert <commit-hash>` | No secret values |
| Nuclear Gateway task dispatch adapter contract | `src/lib/nuclear-gateway-task-dispatch-adapter.ts`; `/api/bridge/nuclear-gateway/task-dispatch-adapter` | Source-only contract and envelope preview for replacing OpenClaw task dispatch transport | Must remain non-executing until live agent receive/audit/rollback proof exists | migrate_to_nuclear_gateway | Direct Agent Line task dispatch adapter contract | Code rollback via `git revert <commit-hash>`; no runtime dispatch behavior changed | No secret values |
| Nuclear Gateway task dispatch no-state proof | `src/lib/nuclear-gateway-task-dispatch-adapter.ts`; preview payload `rollback_or_no_state_proof` | Proves task-dispatch previews are no-state and rollback-neutral before execution is enabled | Must not be mistaken for live receive proof or execution certification | migrate_to_nuclear_gateway | Direct Agent Line task dispatch audit/write proof before execution | Code rollback via `git revert <commit-hash>`; no runtime dispatch behavior changed | No secret values |
| Nuclear Gateway task dispatch audit proof | `src/app/api/bridge/nuclear-gateway/task-dispatch-adapter/route.ts`; `audit_log` internal record | Writes a redacted internal audit entry for authenticated task-dispatch preview packets | Audit write must never include raw message bodies, credentials, cookies, or tokens | migrate_to_nuclear_gateway | Live direct-line receive proof before execution | Code rollback via `git revert <commit-hash>`; audit row is internal proof only and has no runtime side effect | No secret values |
| Nuclear Gateway task dispatch receive trace link | `src/lib/nuclear-gateway-task-dispatch-adapter.ts`; `receive_trace_nonce` / `receive_trace_proof` | Links task-dispatch preview to Direct Agent Line Trace Kit proof without enabling execution | Must require real PASS trace and not allow local-only probe to stand in for live receive proof | migrate_to_nuclear_gateway | Phase 10 cutover certification after live receive proof and Jarvis concurrence | Code rollback via `git revert <commit-hash>`; no runtime dispatch behavior changed | No secret values |
| Nuclear Gateway task dispatch external receive source hardening | `src/lib/nuclear-gateway-task-dispatch-adapter.ts`; `scripts/agent-line-trace.sh`; `receive_trace_proof.external_receive_verified` | Requires non-local trace plus external journal/container receive source before task-dispatch blocker can narrow | Local protected probe or route replay could otherwise be mistaken for live receive proof | migrate_to_nuclear_gateway | Authenticated live Direct Agent Line Trace Kit proof from real receive/response signals | Code rollback via `git revert <commit-hash>`; no runtime dispatch behavior changed | No secret values; auth values are never printed |
