...
const secretNamePattern = [
  'ONEDRIVE_(?:ACCESS_TOKEN|REFRESH_TOKEN)',
  'MICROSOFT_GRAPH_(?:ACCESS_TOKEN|REFRESH_TOKEN)',
  'AZURE_AD_CLIENT_SECRET',
].join('|')
const unsafePattern = new RegExp(`(${secretNamePattern}\\s*[:=]|Bearer\\s+[A-Za-z0-9._-]{20,}|/Users/sosastrike|file://|/var/folders/)`, 'i')
...
const unauth = await request('/api/bridge/agent-zero/onedrive/status')
results.push({
...
const status = await request('/api/bridge/agent-zero/onedrive/status', { auth: true })
results.push({
...
if (status.body?.required_scope !== 'onedrive.upload') failures.push({ name: 'authenticated_status', error: 'missing_onedrive_scope' })
...
const upload = await request('/api/bridge/agent-zero/onedrive/upload-report', {
  method: 'POST',
  auth: true,
  body: { report_id: 'azr_missing_day59', folder: 'Reports' },
})
results.push({
...
if (upload.body?.required_scope !== 'onedrive.upload') failures.push({ name: 'authenticated_upload_stays_blocked', error: 'missing_onedrive_scope' })
...
const payload = {
  ...
  expectation: 'OneDrive readiness and upload proof are authenticated, canonical, folder-aware, scoped, and never fake upload.',
  failures,
  results,
}
