...
export async function POST(request: NextRequest, { params }: { params: Params }) {
  ...
  const result = await callModelRouteApi({ user: auth.user, targetAgentId: agentId, action: 'set_agent_model_default', payload: { route_id: selection.requested_route_id, fallback_policy: selection.fallback_policy } })
  return NextResponse.json(result.payload, { status: result.status, headers: { 'Cache-Control': 'no-store' } })
}
