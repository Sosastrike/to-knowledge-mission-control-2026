  ...
  try {
    ...
    db.transaction(() => {
      ...
      `).run(
        ...
        row.target,
        row.target_key,
        JSON.stringify({ no_execution_enabled: true, decision_only: true }),
        row.correlation_id,
      )
    })()
    ...
    return NextResponse.json({
      ...
      accepted_for_execution: false,
      no_connector_writes_enabled: true,
      next_action: 'Approval decision is recorded. A separate owner-approved execution runner is still required before any protected action can run.',
    })
  } catch (error) {
    return NextResponse.json({
      ok: false,
