...
const BUTTONS: ButtonContract[] = [
  ...
  { route: 'brain-sync', label: 'Build-Wiki farmer logs', endpoint: '/api/bridge/brain-sync/build-wiki/logs', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Read-only redacted tail of the Build-Wiki/Farmer docs sync log.' },
  ...
  { route: 'gateway', label: 'OCR Specialist direct line', endpoint: '/api/gateway/extraction/ai', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Read-only OCR Specialist direct-line/proxy status. OCR is visible in Gateway but activation is blocked until Jarvis concurrence.' },
  { route: 'gateway', label: 'AI extraction proxy', endpoint: '/api/gateway/extraction/ai', method: 'POST', state: 'OWNER_APPROVAL_REQUIRED', credential_names: [], approval_required: true, audit_required: true, owner: 'Codex', note: 'Server-side proxy prevents browser CORS/API-key exposure. POST returns HTTP 423 until Jarvis concurrence, approval/audit persistence, and scoped runner wiring exist.' },
