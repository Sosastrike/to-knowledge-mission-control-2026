...
const BUTTONS: ButtonContract[] = [
  ...
  { route: 'brain-sync', label: 'Brain Sync source status', endpoint: '/api/bridge/brain-sync/status', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Read-only Tony memory, MemPalace, Obsidian, Graphify, and Brain Sync visibility. No memory writes, no protected memory changes, no DB migration.' },
  ...
  { route: 'brain-sync', label: 'Build-Wiki farmer logs', endpoint: '/api/bridge/brain-sync/build-wiki/logs', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Read-only redacted tail of the OpenCloud docs farmer log.' },
  { route: 'brain-sync', label: 'Build-Wiki request run now', endpoint: '/api/bridge/brain-sync/build-wiki/run-now', method: 'POST', state: 'OWNER_APPROVAL_REQUIRED', credential_names: [], approval_required: true, audit_required: true, owner: 'Codex', note: 'Creates a scoped Tony-to-Telegram approval request only; it does not start the farmer service.' },
  ...
  { route: 'brain-sync', label: 'Build-Wiki dispatch approved run', endpoint: '/api/bridge/brain-sync/build-wiki/run-now/:id/dispatch', method: 'POST', state: 'OWNER_APPROVAL_REQUIRED', credential_names: [], approval_required: true, audit_required: true, owner: 'Codex', note: 'Exact-scope dispatch only after approval: systemctl --user start opencloud-docs-farmer.service. No broad connector execution.' },
  ...
  { route: 'agent-network', label: 'Cost/rate governance', endpoint: '/api/bridge/costs', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Read-only token/cost visibility and rate-limit policy. It does not enforce budgets, change provider routes, or enable execution.' },
  { route: 'agent-network', label: 'Owner gates / blockers', endpoint: '/api/bridge/owner-gates', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Read-only summary of owner approvals, credential blockers, and backend gates. It never creates approvals or enables execution.' },
  ...
  { route: 'agent-network', label: 'Approval queue', endpoint: '/api/bridge/approval-requests', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Read-only approval queue. Persistence is connected when the owner-approved migration is applied; protected execution remains locked.' },
  { route: 'agent-network', label: 'Approve request in web UI', endpoint: '/api/bridge/approval-requests/:id/approve', method: 'POST', state: 'OWNER_APPROVAL_REQUIRED', credential_names: [], approval_required: true, audit_required: true, owner: 'Codex', note: 'Canonical owner decisions are Telegram-only through Tony inline Approve/Deny buttons. Mission Control shows the state but blocks web approval with HTTP 423 to prevent duplicate approval systems.' },
  { route: 'agent-network', label: 'Deny request in web UI', endpoint: '/api/bridge/approval-requests/:id/deny', method: 'POST', state: 'OWNER_APPROVAL_REQUIRED', credential_names: [], approval_required: true, audit_required: true, owner: 'Codex', note: 'Canonical owner decisions are Telegram-only through Tony inline Approve/Deny buttons. Mission Control shows the state but blocks web denial with HTTP 423 to prevent duplicate approval systems.' },
  { route: 'agent-network', label: 'Add agent', endpoint: '/api/agents', method: 'POST', state: 'OWNER_APPROVAL_REQUIRED', credential_names: [], approval_required: true, audit_required: true, owner: 'Codex', note: 'Production create remains disabled until owner approves scoped execution runners, even when approval/audit persistence exists.' },
