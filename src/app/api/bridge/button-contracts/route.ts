...
const BUTTONS: ButtonContract[] = [
  ...
  { route: 'agent-network', label: 'Approve request', endpoint: '/api/bridge/approval-requests/:id/approve', method: 'POST', state: 'OWNER_APPROVAL_REQUIRED', credential_names: [], approval_required: true, audit_required: true, owner: 'Codex', note: 'Approval decisions require an existing persisted request. Decisions are recorded only after owner action; execution remains separately locked.' },
  { route: 'agent-network', label: 'Deny request', endpoint: '/api/bridge/approval-requests/:id/deny', method: 'POST', state: 'OWNER_APPROVAL_REQUIRED', credential_names: [], approval_required: true, audit_required: true, owner: 'Codex', note: 'Approval decisions require an existing persisted request. Decisions are recorded only after owner action; execution remains separately locked.' },
  ...
  { route: 'zapier', label: 'Search HeyGen tools', endpoint: '/api/bridge/zapier/tools/search?q=heygen', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Canonical preflight lookup before asking for video/HeyGen credentials. No invocation.' },
