...
const BUTTONS: ButtonContract[] = [
  ...
  { route: 'brain-sync', label: 'Brain Sync source status', endpoint: '/api/bridge/brain-sync/status', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Read-only Agent Zero Brain, MemPalace, Obsidian, Graphify, and Build-Wiki visibility. ClaudeClaw dashboard connection adds upstream enrichment when available; local read-only adapters still report truthful status when upstream is absent. No memory writes, no protected memory changes, no DB migration.' },
  { route: 'brain-sync', label: 'Knowledge report', endpoint: '/api/bridge/brain-sync/knowledge-report', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Safe owner-facing source-state knowledge report generated from Brain Sync status. It exposes counts and blockers while omitting local paths, secret values, private record dumps, and memory writes.' },
  { route: 'brain-sync', label: 'Obsidian adapter status', endpoint: '/api/bridge/agent-zero/obsidian', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Read-only Agent Zero Obsidian adapter status. The endpoint reports SERVICE_DOWN when the configured vault is unavailable and never exposes the raw vault path.' },
  { route: 'brain-sync', label: 'Obsidian adapter safe note search', endpoint: '/api/bridge/agent-zero/obsidian', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Uses action=search to search safe note previews only when the vault is visible. Returns an exact blocker when unavailable; no writes and no raw private dumps.' },
  { route: 'brain-sync', label: 'Obsidian adapter safe note read', endpoint: '/api/bridge/agent-zero/obsidian', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Uses action=read to read one safe note preview by path or title through the adapter. Content is bounded and redacted; writes stay disabled.' },
  { route: 'brain-sync', label: 'Obsidian adapter note summary', endpoint: '/api/bridge/agent-zero/obsidian', method: 'GET', state: 'READ_ONLY', credential_names: [], approval_required: false, audit_required: false, owner: 'Codex', note: 'Uses action=summarize to summarize one safe note through the read-only adapter. Returns exact blocked state if the vault or note is unavailable.' },
  { route: 'brain-sync', label: 'Obsidian note write actions', endpoint: '/api/bridge/agent-zero/execute', method: 'POST', state: 'OWNER_APPROVAL_REQUIRED', credential_names: [], approval_required: true, audit_required: true, owner: 'Codex', note: 'Obsidian note create/update/tag/link actions are only available through Agent Zero execution with a scoped Bridge Session and audit record.' },
