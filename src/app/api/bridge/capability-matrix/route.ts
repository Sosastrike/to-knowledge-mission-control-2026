...
export async function GET(request: NextRequest) {
  ...
  const agents: AgentCapability[] = [
    ...
    {
      id: 'tony_legacy',
      display_name: 'Tony Legacy',
      role: 'retired archived commander surface',
      status: 'disabled',
      execution_permission: 'disabled',
      available_models: [],
      available_tools: ['archived audit/report history only'],
      available_skills: ['legacy metadata only'],
      available_integrations: [],
      available_mcps: [],
      provider_routes: ['retired=true', 'hidden_from_normal_owner_view=true'],
      approval_gates: [],
      restrictions: ['Must not answer as active commander', 'Must not own Brain Sync', 'Must not own new reports'],
      memory_brain_sync_status: 'retired/archived; Agent Zero is the primary brain operator',
      harness_event_routing_status: 'disabled as commander',
      cost_rate_limits: [],
      blockers: ['ClaudeClaw may remain as transport/backend until dependency map is complete'],
      next_action: 'Keep for rollback/audit only; do not select Tony as commander.',
      can_execute: false,
    },
    {
      id: 'hermes',
      ...
      available_integrations: ['No production credentials', 'No legacy memory connection'],
