...
export async function GET(request: NextRequest) {
  ...
  const agents: AgentCapability[] = [
    ...
    },
    {
      ...
      available_skills: ['OpenClaw+ shared skills/runtime layer', 'Hermes shared skills', 'Skill creation guidance', 'workflow improvement recommendations'],
      ...
      provider_routes: ['lieutenant=hermes', 'sandbox_home=hermes-sandbox'],
    ...
    },
    {
      id: 'pi',
      display_name: 'Pi',
      role: 'dispatcher / route optimizer candidate (shadow)',
      status: 'degraded',
      execution_permission: 'read_only',
      available_models: [],
      available_tools: ['route recommendation matrix', 'dispatcher policy checks'],
      available_skills: ['route selection and escalation guidance'],
      available_integrations: ['Mission Control registry visibility only'],
      available_mcps: ['none for direct execution'],
      provider_routes: ['dispatcher=pi', 'mode=shadow_advisory_only'],
      approval_gates: ['Bridge Session for protected actions', 'owner approval for execution promotion'],
      restrictions: ['No execution', 'No writes', 'No connector sends/uploads', 'No fake completion'],
      memory_brain_sync_status: 'recommendation-only, no write',
      harness_event_routing_status: 'advisory handoff to Agent Zero/Hermes/Paperclip',
      cost_rate_limits: ['No runtime dispatch execution until promoted'],
      blockers: ['pi_shadow_dispatcher_not_execution_capable'],
      next_action: 'Keep Pi visible as a dispatcher candidate with advisory-only recommendations.',
      can_execute: false,
    },
    {
      id: 'spaceagent',
      ...
      status: 'degraded',
      execution_permission: 'read_only',
      available_models: ['provider-agnostic read-only research packet generation'],
      available_tools: ['Playwright MCP status', 'YouTube metadata route', 'Firecrawl readiness status'],
      available_skills: ['research packet generation', 'evidence capture'],
      available_integrations: ['Playwright MCP local-only', 'YouTube limited', 'Firecrawl blocked until credential/backend'],
      available_mcps: ['Playwright MCP read-only probes'],
      provider_routes: ['specialist=spaceagent', 'playwright=local_only'],
      approval_gates: ['Bridge Session for interactive browser actions', 'credentialed provider writes'],
      restrictions: ['No broad crawl', 'No private page login bypass', 'No external writes'],
      memory_brain_sync_status: 'read-only research output',
      harness_event_routing_status: 'delegated by Agent Zero/Pi',
      cost_rate_limits: ['no uncontrolled crawl loops'],
      blockers: ['firecrawl_credential_required', 'youtube_transcript_connector_not_proven'],
      next_action: 'Promote from LIMITED when transcript and credentialed read-only research proofs pass.',
      can_execute: false,
    },
    {
      id: 'paperclip',
      ...
      execution_permission: 'owner_approval_required',
      available_models: [],
      available_tools: ['status route', 'companies route', 'agents route', 'issues route'],
      available_skills: ['task orchestration contracts', 'workforce tracking'],
      available_integrations: ['local/Tailnet sandbox endpoint when running'],
      available_mcps: ['none for direct mutation'],
      provider_routes: ['workforce=paperclip', 'placement=before_openclaw_plus'],
      approval_gates: ['Bridge Session for mutations', 'owner session/login for live workforce proof'],
      restrictions: ['No writes without Bridge Session', 'No public exposure', 'No fake task creation'],
      memory_brain_sync_status: 'task metadata only when service/auth is available',
      harness_event_routing_status: 'routes from Agent Zero/Pi/Hermes into workforce queue',
      cost_rate_limits: ['no autonomous budget writes'],
      blockers: ['paperclip_sandbox_service_not_running', 'paperclip_owner_session_required'],
      next_action: 'Recover local/Tailnet service and owner session proof before enabling live workforce reads.',
      can_execute: false,
    },
    {
      id: 'openclaw_gateway',
  ...
  const payload = {
    ...
    shared_skill_runtime: agentZeroContext.skills.shared_runtime,
    shared_skill_sources: agentZeroContext.skills.sources,
    approval_gates: [
      'credentials',
      'memory writes',
      'governance changes',
