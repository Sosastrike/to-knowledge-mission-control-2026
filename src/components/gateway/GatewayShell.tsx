...
export const AGENT_INTERFACE_LINKS: ReadonlyArray<AgentInterfaceLink> = [
  ...
  },
  {
    ...
    authRequired: true,
    status: 'blocked',
    blocker: 'agentmail_owner_sso_or_api_key_required · agentmail_inbox_not_provisioned · agentmail_inbox_credential_required; sends remain owner-approved Action Bridge gated.',
    nextFix: 'Open AgentMail Local Control, connect AgentMail, then preview/provision inbox registry and scoped credentials. Sends stay approval and Action Bridge gated.',
    buttons: {
      ui: enabled('/agentmail'),
      config: enabled(GATEWAY_TOOLS_ROUTE),
      brain: enabled(GATEWAY_BRAIN_ROUTE),
      chat: disabled('AgentMail send/reply is disabled until runtime access, provisioned inboxes, scoped credentials, owner approval, and Action Bridge proof exist.'),
      tools: enabled(GATEWAY_TOOLS_ROUTE),
      health: enabled(GATEWAY_TOOLS_ROUTE),
    },
  },
