...
export function buildModelCapabilityRegistry(): { generated_at: string; providers: ModelCapabilityRecord[]; summary: Record<string, unknown> } {
  ...
  const providers = ids.map((providerId) => {
    ...
    return {
      ...
      execution_enabled: false,
      allowed_agents: ['agent-zero', 'tony', 'hermes', 'researcher', 'operator', 'builder', 'future-agents'],
...
export function buildBaselineSnapshot() {
  ...
  return {
    ...
    confirmed_facts: [
      'Mission Control exposes Gateway, Agent Hub, provider, task, and Agent Zero owner-facing shell route surfaces.',
      'Sophia is a human assistant/operator and is not part of the software execution path for this recovery batch.',
