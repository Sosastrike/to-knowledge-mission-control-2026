...
describe('Gateway mini-agent operating system', () => {
  it('keeps the core hierarchy intact and adds Pi without replacing Agent Zero', () => {
    ...
    const legacyControllerNodes = registry.nodes.filter((node) => node.id === 'legacy_deleted_controller')
    ...
    expect(os.registry.legacy_deleted_controller_active_authority).toBe(false)
