...
import { recordGatewayModelUsage } from '@/lib/gateway-telemetry-producer'
...
type ProviderId = 'openrouter' | 'openai' | 'gemini' | 'groq' | 'nvidia' | 'xai_grok'
...
function canonicalProvider(value: unknown): ProviderId | null {
  ...
  if (id === 'gpt' || id === 'codex') return 'openai'
  if (id === 'openrouter' || id === 'openai' || id === 'gemini' || id === 'groq' || id === 'nvidia' || id === 'xai_grok') return id
  return null
}
