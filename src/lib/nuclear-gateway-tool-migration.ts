...
export const NUCLEAR_GATEWAY_TOOL_MIGRATION_ENTRIES: NuclearGatewayToolMigrationEntry[] = [
  ...
  entry({
    id: 'openclaw.task_dispatch.new_session',
    label: 'Mission Control task dispatch via OpenClaw new-session agent call',
    source_dependency: 'src/lib/task-dispatch.ts can invoke `runOpenClaw gateway call agent --expect-final` for newly assigned tasks',
    current_openclaw_role: 'legacy_task_dispatch_transport',
    migration_target: 'POST /api/bridge/agent-routing/send with canonical message envelope and certified direct agent line ownership',
    ...
    execution_enabled: false,
    openclaw_allowed_role: 'not_allowed',
    rollback_note: 'Keep existing dispatcher unchanged until the direct-line task dispatch adapter passes live receive proof; revert only the registry entry if needed.',
    blocker: 'direct_line_task_dispatch_adapter_required',
  }),
  entry({
    id: 'openclaw.task_dispatch.target_session',
    label: 'Mission Control task dispatch via OpenClaw targeted session send',
    source_dependency: 'src/lib/task-dispatch.ts can call `chat.send` when task metadata includes target_session',
    current_openclaw_role: 'legacy_target_session_transport',
    migration_target: 'Direct Agent Line session handoff with target session provenance, route trace, audit, and rollback/no-state proof',
    ...
    execution_enabled: false,
    openclaw_allowed_role: 'not_allowed',
    rollback_note: 'No runtime behavior changed; keep legacy target-session dispatch until direct-line handoff is certified.',
    blocker: 'target_session_direct_line_handoff_required',
  }),
  entry({
    id: 'openclaw.task_review.aegis',
    label: 'Aegis quality review via OpenClaw agent call',
    source_dependency: 'src/lib/task-dispatch.ts can invoke `runOpenClaw gateway call agent` for Aegis reviews',
    current_openclaw_role: 'legacy_quality_review_transport',
    migration_target: 'Nuclear Gateway review adapter with explicit reviewer identity and no OpenClaw conversation ownership',
    ...
    execution_enabled: false,
    openclaw_allowed_role: 'not_allowed',
    rollback_note: 'No quality review runtime change made; migrate only after exact-scope review adapter proof exists.',
    blocker: 'quality_review_direct_line_adapter_required',
  }),
  entry({
    id: 'openclaw.task_broadcast',
    label: 'Task broadcast to agent sessions via OpenClaw sessions_send',
    source_dependency: 'src/app/api/tasks/[id]/broadcast/route.ts sends subscriber messages through OpenClaw session transport',
    current_openclaw_role: 'legacy_task_broadcast_transport',
    migration_target: 'Nuclear Gateway broadcast adapter that fans out through direct agent lines and writes visible trace proof',
    ...
    execution_enabled: false,
    openclaw_allowed_role: 'not_allowed',
    rollback_note: 'No broadcast runtime change made; direct-line broadcast remains blocked until adapter proof and rollback exist.',
