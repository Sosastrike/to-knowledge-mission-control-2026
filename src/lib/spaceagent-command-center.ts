...
export function buildSpaceAgentStatus() {
  ...
  return {
    ...
    live_proof_readiness_route: '/api/bridge/spaceagent/live-proof-readiness',
    final_certification_route: '/api/bridge/spaceagent/final-certification',
...
export function buildSpaceAgentLiveProofReadiness() {
  return {
    ...commonInvariants(),
    ...
    source_ready: true,
    live_certification_ready: false,
    restart_not_attempted: true,
    requires_service_restart: true,
    requires_authenticated_owner_session: true,
    direct_line_trace_command: '/home/tony/agent-line-trace.sh --agent spaceagent --local-probe',
    browser_target_after_restart: '/gateway/agent-hub',
    readiness_routes: {
      login: '/login',
      status: '/api/bridge/spaceagent/status',
      certification_proof: '/api/bridge/spaceagent/certification-proof',
      quality_scorecard: '/api/bridge/spaceagent/quality-scorecard',
      live_trace: '/api/bridge/agent-routing/trace/live?agent=spaceagent',
      trace_probe: '/api/bridge/agent-routing/trace/probe',
      visible_task: '/api/tasks/149/events',
    },
    ...
    post_restart_validation: [
      { id: 'login_200', command: 'curl -I http://127.0.0.1:3337/login', expected: '200' },
      { id: 'protected_spaceagent_401', command: 'curl -I http://127.0.0.1:3337/api/bridge/spaceagent/live-proof-readiness', expected: '401 unauthenticated' },
      { id: 'service_cwd_not_deleted', command: 'readlink /proc/$(systemctl show mission-control.service -p MainPID --value)/cwd', expected: '/home/tony/mission-control/.next/standalone' },
      { id: 'authenticated_browser_trace', command: 'owner authenticated browser presses SpaceAgent Trace Direct Line', expected: 'response_received true and OpenCloud false' },
    ],
    blockers: [
      'mission_control_service_stale_next_bundle_restart_required',
      'authenticated_owner_browser_session_required_for_final_trace',
    ],
    proof: {
      source_surfaces_ready: true,
      no_opencloud_intermediary: true,
      no_external_execution: true,
      no_production_write: true,
      credential_values_exposed: false,
      raw_session_values_exposed: false,
      rollback_id: 'no_state_spaceagent_live_proof_readiness',
      rollback_command: 'Remove /api/bridge/spaceagent/live-proof-readiness and Agent Hub live-proof readiness metadata; no production state changed.',
    },
    writes_enabled: false,
    execution_enabled: false,
    external_execution_enabled: false,
...
export function buildSpaceAgentFinalCertification() {
  const certification = buildSpaceAgentCertificationProof()
  const liveProof = buildSpaceAgentLiveProofReadiness()
  const quality = buildSpaceAgentQualityScorecard()
  ...
  return {
    ...
    completion_percent_source_verified: 92,
    may_mark_ready_now: false,
    source_evidence: {
      direct_line_registry: 'SOURCE_VERIFIED',
      agent_hub_card: 'SOURCE_VERIFIED',
      gateway_graph: 'SOURCE_VERIFIED',
      pipeline_templates: 'SOURCE_VERIFIED',
      brain_bridge_read_only: 'SOURCE_VERIFIED',
      tool_map_gateway_brokered: 'SOURCE_VERIFIED',
      internal_records: 'SOURCE_VERIFIED',
      unsafe_action_refusal: 'SOURCE_VERIFIED',
      certification_proof_route: certification.route,
      quality_scorecard_route: quality.route,
      live_proof_readiness_route: liveProof.route,
    },
    acceptance_criteria: [
      { id: 'agent_hub_visible', state: 'SOURCE_VERIFIED' },
      { id: 'gateway_graph_visible', state: 'SOURCE_VERIFIED' },
      { id: 'command_registry_visible', state: 'SOURCE_VERIFIED' },
      { id: 'direct_line_trace', state: 'PENDING_AUTHENTICATED_LIVE_PROOF' },
      { id: 'opencloud_hidden_intermediary_refused', state: 'SOURCE_VERIFIED' },
      { id: 'gateway_state_read', state: 'SOURCE_VERIFIED' },
      { id: 'brain_state_read', state: 'SOURCE_VERIFIED' },
      { id: 'visible_task_event_write', state: 'SOURCE_VERIFIED_INTERNAL_ONLY' },
      { id: 'report_draft', state: 'SOURCE_VERIFIED_INTERNAL_ONLY' },
      { id: 'jarvis_concurrence_request', state: 'SOURCE_VERIFIED_INTERNAL_ONLY' },
      { id: 'approved_tools_visible', state: 'SOURCE_VERIFIED' },
      { id: 'unsafe_broad_action_refused', state: 'SOURCE_VERIFIED' },
      { id: 'audit_no_state_proof', state: 'SOURCE_VERIFIED' },
      { id: 'rollback_path', state: 'SOURCE_VERIFIED' },
      { id: 'no_secret_proof', state: 'SOURCE_VERIFIED' },
      { id: 'mission_control_service_active', state: 'ACTIVE_STALE_BUNDLE_RESTART_REQUIRED' },
      { id: 'authenticated_browser_gateway_load', state: 'PENDING_SERVICE_RESTART' },
    ],
    blockers: [
      'mission_control_service_stale_next_bundle_restart_required',
      'authenticated_owner_browser_session_required_for_final_trace',
    ],
    next_safe_lane: 'Hold final ready status until service restart and authenticated SpaceAgent direct-line proof pass; continue source-only proof packets.',
    proof: {
      ...
      credential_values_exposed: false,
      raw_session_values_exposed: false,
      rollback_id: 'no_state_spaceagent_final_certification',
      rollback_command: 'Remove /api/bridge/spaceagent/final-certification and Agent Hub final certification metadata; no production state changed.',
    },
    writes_enabled: false,
    execution_enabled: false,
    external_execution_enabled: false,
